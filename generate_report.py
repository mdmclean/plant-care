...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }}
    ...
    .need-chip.water   {{ background: var(--warn-bg);   color: var(--warn-text);   border-color: var(--warn); }}
    .need-chip.fert    {{ background: var(--info-bg);   color: var(--info-text);   border-color: var(--info); }}
    .need-chip.unknown {{ background: var(--danger-bg); color: var(--danger-text); border-color: var(--danger); }}
    .need-chip.allgood {{ background: var(--ok-bg);     color: var(--ok-text); }}
    ...
    /* A button this plant actually needs today: full-color, drawing the eye. */
    .chk.need {{ opacity: 1; filter: none; }}
    .chk.need[data-act="w"], .chk.need[data-act="wet"] {{
...
function rowHTML(p, i) {{
  ...
  if (!chips.length) chips.push('<span class="need-chip allgood">✓ Nothing due</span>');
  const needs = `<div class="row-needs">${{chips.join('')}}</div>`;

  // Highlight only the buttons relevant to what this plant needs.
  const cls = (act, done) => {{
